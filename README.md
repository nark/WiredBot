...
git clone https://github.com/nark/WiredBot.git
cd WiredBot

# Build the bot
swift build --product WiredBot -c release

# Binary is located at
.build/release/WiredBot
```

Copy the binary and the protocol spec file:

```bash
sudo cp .build/release/WiredBot /usr/local/bin/wiredbot
sudo mkdir -p /usr/share/wiredbot
swift package resolve
sudo cp .build/checkouts/WiredSwift/Sources/WiredSwift/Resources/wired.xml /usr/share/wiredbot/
```
